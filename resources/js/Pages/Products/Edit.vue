<template>
    <MainLayout>
        ...
        <div class="flex items-center mb-8">
            <div>
                <h2 class="font-black text-2xl text-gray-900 tracking-tight">{{ $t('Edit Product') }}</h2>
                <p class="text-sm font-bold text-orange-500 mt-1 tracking-wide">{{ product.name }}</p>
            </div>
        </div>
        ...
        <div class="bg-white/60 backdrop-blur-xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-8 max-w-3xl relative overflow-hidden">
            <!-- Decorative blobs -->
            <div class="absolute -top-24 -right-24 w-48 h-48 bg-gradient-to-br from-indigo-400 to-indigo-600 rounded-full blur-3xl opacity-5"></div>
            <div class="absolute -bottom-24 -left-24 w-48 h-48 bg-gradient-to-tr from-orange-400 to-orange-600 rounded-full blur-3xl opacity-10"></div>

            <form @submit.prevent="submit" class="relative z-10">
                <div class="space-y-6">
                    <div>
                        <label for="name" class="block text-xs font-black text-gray-900 uppercase tracking-widest mb-2 pl-1">Product Name</label>
                        <input 
                            type="text" 
                            id="name" 
                            v-model="form.name" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="category_id" class="block text-xs font-black text-gray-900 uppercase tracking-widest mb-2 pl-1">Category</label>
                            <div class="relative">
                                <select 
                                    id="category_id" 
                        ...
                        <div>
                            <label for="price" class="block text-xs font-black text-gray-900 uppercase tracking-widest mb-2 pl-1">Product Price (RM)</label>
                            <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="quantity" class="block text-xs font-black text-gray-900 uppercase tracking-widest mb-2 pl-1">Quantity Stock</label>
                            <input 
                                type="number" 
                                id="quantity" 
                         ...
                         <div>
                            <div class="flex items-center justify-between mb-2 pl-1">
                                <label class="block text-xs font-black text-gray-900 uppercase tracking-widest">Product Image</label>
                                <div class="flex bg-gray-100 p-1 rounded-lg">
                                    <button type="button" @click="imageMode = 'url'" :class="imageMode === 'url' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-500'" class="px-3 py-1 text-[10px] font-bold rounded-md transition-all">URL</button>
                                    <button type="button" @click="imageMode = 'upload'" :class="imageMode === 'upload' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-500'" class="px-3 py-1 text-[10px] font-bold rounded-md transition-all">UPLOAD</button>
                                </div>
                            </div>
...
const submit = () => {
    // We use POST with _method: PUT because partial multipart forms (with files) don't work reliably with PUT in PHP
